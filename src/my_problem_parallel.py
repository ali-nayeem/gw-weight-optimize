...
class GW(Problem):
    def __init__(self, sample_id: int, sample_path: str, feature_names: list, init_weight, cpu_cores):
    ...
    def _evaluate_serial(self, x, out, *args, **kwargs):
...
if __name__ == '__main__':
    eval = EvaluatePop()
    prob = GW(0, Input.sample_path, Input.input_features_label, Input.ahp_weight, cpu_cores=8)
    x = []
    for f in Input.features_order:
        x.append(Input.ahp_weight[f])
    prob.evaluate(np.array(x), None)
    pop = prob.initialize(100, np.array(x), rand_percentage=0.1)
    X =  pop.get("X")
